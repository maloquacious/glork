...
// GLORK: Snowflake Method wireframe (single-file)
...
const demoNotes: Record<string, string> = {
  overview:
    "Start here. Capture your elevator pitch, genre, target audience, && comps.",
  outline:
    "Hierarchical outline. Drag to reorder. Promote/demote to change depth.",
  characters:
    "Cast list with goals, conflicts, stakes. Track POV && appearance counts.",
  scenes:
    "Beat-by-beat planner. Each scene has purpose, conflict, outcome, && hooks.",
};
        ...
        {projects.map((p) => (
          <li key={p.id}>
            <button
              ...
              className={`w-full text-left px-3 py-2 rounded-xl hover:bg-gray-100 ${selectedId === p.id ? "bg-gray-100" : ""}`}
            >
    ...
    <div className="flex gap-1 border-b px-4 bg-white/60 backdrop-blur sticky top-12 z-10">
      {tabs.map((t) => (
        <button
          key={t}
          onClick={() => onChange(t)}
          className={`px-4 py-2 text-sm rounded-t-xl border-x border-t -mb-px ${active === t ? "bg-white" : "bg-gray-50 text-gray-600"}`}
        >
          {t}
        </button>
      ))}
            ...
            {activeTab === "Overview" && (
              <div className="grid gap-4 md:grid-cols-2">
                <PanelCard title="Project Brief">
                  <p className="text-sm text-gray-700 leading-relaxed">{demoNotes.overview}</p>
